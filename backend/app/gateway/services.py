...
# Keys honored only for internally-authenticated callers (scheduler and bounded
# internal-workflow paths).
...
_CONTEXT_INTERNAL_CALLER_KEYS: frozenset[str] = frozenset(
    {
        "non_interactive",
        "tool_call_limit",
        "prevent_duplicate_failed_reads",
        "persistent_run_reminder",
        "max_redundant_read_file_calls_without_write",
        "blocked_tool_names",
        "allowed_bash_command_prefixes",
        "bash_call_limit",
        "skip_memory",
        "skip_summarization",
        "bug_specialist_finalization_enabled",
        "bug_ui_analysis",
        "bug_ui_problem_focus",
    }
)

# Internal-only values consumed directly from ToolRuntime.context.  Unlike the
# middleware/configuration controls above, these never need to be copied into
# ``configurable`` (which is persisted with checkpoints).
_CONTEXT_INTERNAL_RUNTIME_ONLY_KEYS: frozenset[str] = frozenset({BUG_WORKBENCH_OWNER_USER_ID_CONTEXT_KEY})
...
async def start_run(
    ...
    try:
        ...
        from deerflow.runtime.main_assistant_policy import apply_main_assistant_run_policy

        apply_main_assistant_run_policy(config, body.config)
