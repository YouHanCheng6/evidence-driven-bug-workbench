...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    async with langgraph_runtime(app, startup_config):
        ...
        # Must run AFTER langgraph_runtime so the thread metadata store is available.
        ...
        try:
            from app.gateway.main_agent_bug_batch import resume_pending_bug_batches

            thread_store = getattr(app.state, "thread_store", None)
            if thread_store is not None:
                resumed_batches = await resume_pending_bug_batches(thread_store)
                if resumed_batches:
                    logger.info("Resumed %d persisted Bug Workbench batch(es)", resumed_batches)
        except Exception:
            logger.exception("Could not resume persisted Bug Workbench batches")

        try:
            from app.gateway.services import launch_scheduled_thread_run
            from app.scheduler import ScheduledTaskService
            from app.channels.service import deliver_scheduled_run_result
            ...
            if getattr(app.state, "scheduled_task_repo", None) is not None and getattr(app.state, "scheduled_task_run_repo", None) is not None:
                scheduled_task_service = ScheduledTaskService(
                    ...
                    deliver_result=deliver_scheduled_run_result,
                )
...
This gateway provides runtime endpoints for agent runs plus custom endpoints for models, MCP configuration, skills, and artifacts.
    ...
    app.include_router(bug_workflows.router)
