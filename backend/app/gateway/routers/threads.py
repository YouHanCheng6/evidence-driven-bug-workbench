...
from app.gateway.bug_workflow import cancel_workflow_tasks
...
# Global Feishu channel deployments create their service conversations under
# this owner. They are intentionally exposed through the narrowly-scoped
# maintenance routes below, never through the ordinary owner-scoped thread APIs.
_FEISHU_SERVICE_THREAD_OWNER = "default"
...
class ThreadClearContextResponse(BaseModel):
    """Result of clearing one thread's retained conversation state."""

    success: bool
    removed_events: int


def _is_feishu_channel_thread(record: dict[str, Any]) -> bool:
    """Return whether a thread belongs to DeerFlow's shared Feishu channel."""
    metadata = record.get("metadata")
    source = metadata.get("channel_source") if isinstance(metadata, dict) else None
    return isinstance(source, dict) and source.get("type") == "im_channel" and source.get("provider") == "feishu"


def _is_feishu_bug_workflow(record: dict[str, Any]) -> bool:
    """Return whether a record is a Feishu-originated Bug Workbench task.

    Each Bug Workbench run has one root record. Its router, analysis, and
    note-writer threads are deliberately not channel conversations and must
    never appear in the team-maintenance UI.
    """
    metadata = record.get("metadata")
    ...
    if not isinstance(workflow, dict):
        return False
    channel_key = metadata.get("bug_workflow_channel_key") or workflow.get("channel_key")
    return isinstance(channel_key, str) and channel_key.startswith("feishu\x1f")


def _bug_workflow_thread_ids(record: dict[str, Any]) -> tuple[str, ...]:
    """Return the root plus every hidden worker owned by a Bug task.

    A Bug root may own router, specialist, review, note, and repair threads.
    UI deletion treats that set as one task even though workers stay hidden.
    """
    root_id = record.get("thread_id")
    ...
    if isinstance(workflow, dict):
        for key in (
            ...
            thread_id = workflow.get(key)
            if isinstance(thread_id, str) and thread_id and thread_id not in thread_ids:
                thread_ids.append(thread_id)
    return tuple(thread_ids)


def _shared_feishu_record_thread_ids(record: dict[str, Any]) -> tuple[str, ...]:
    """Return one conversation or the complete hidden-worker Bug task."""
    return _bug_workflow_thread_ids(record)


async def _cancel_bug_workflow_before_delete(
    request: Request,
    record: dict[str, Any],
    *,
    user_id: str,
) -> None:
    """Durably block side effects, then stop agent runs and local wrappers."""
    metadata = record.get("metadata")
    workflow = metadata.get("bug_workflow") if isinstance(metadata, dict) else None
    if not isinstance(workflow, dict):
        return
    workflow_id = record.get("thread_id")
    ...
    cancelled = {
        **workflow,
        "status": "cancelled",
        "cancel_reason": "用户删除 Bug 工作台会话",
    }
    thread_store = get_thread_store(request)
    await thread_store.update_metadata(
        workflow_id,
        {"bug_workflow": cancelled},
        user_id=user_id,
    )

    run_manager = get_run_manager(request)
    owned_thread_ids = set(_bug_workflow_thread_ids(record))
    run_ids: list[str] = []
    for key in (
        "router_run_id",
        "analysis_run_id",
        ...
        run_id = workflow.get(key)
        if not isinstance(run_id, str) or not run_id or run_id in run_ids:
            continue
        run_ids.append(run_id)
    for owned_thread_id in owned_thread_ids:
        for run in await run_manager.list_by_thread(
            owned_thread_id,
            user_id=user_id,
            limit=100,
        ):
    ...
    local_run_tasks: list[asyncio.Task[Any]] = []
    for run_id in run_ids:
        run = await run_manager.get(run_id, user_id=user_id)
        if run is None or getattr(run, "thread_id", None) not in owned_thread_ids:
            continue
        await run_manager.cancel(run_id, action="interrupt")
        task = getattr(run, "task", None)
        if isinstance(task, asyncio.Task) and not task.done():
            local_run_tasks.append(task)

    if local_run_tasks:
        try:
            await asyncio.wait_for(
                asyncio.gather(*local_run_tasks, return_exceptions=True),
                timeout=5.0,
            )
        except TimeoutError as exc:
            raise HTTPException(
                status_code=409,
                detail="Bug workflow is still stopping; retry deletion shortly",
            ) from exc

    await cancel_workflow_tasks(workflow_id)


def _shared_feishu_owner_ids() -> tuple[str, ...]:
    """Return the service owners whose Feishu records are team-managed.

    Normal channel chat uses the global service owner. Bug Workbench may use a
    separately configured shared owner so its ZenTao credentials and specialist
    agents are not distributed to individual Feishu members.
    """
    owner_ids = [_FEISHU_SERVICE_THREAD_OWNER]
    try:
        ...
        configured_owner = feishu.get("bug_workbench_owner_user_id") if isinstance(feishu, dict) else None
        if isinstance(configured_owner, str) and configured_owner.strip():
            owner_ids.append(configured_owner.strip())
    except Exception:
        logger.warning("Unable to read Bug Workbench service owner for Feishu maintenance", exc_info=True)
    return tuple(dict.fromkeys(owner_ids))


def _is_shared_feishu_record(record: dict[str, Any]) -> bool:
    """Return whether this is a displayable team Feishu record."""
    return _is_feishu_channel_thread(record) or _is_feishu_bug_workflow(record)


async def _get_shared_feishu_thread(request: Request, thread_id: str) -> tuple[dict[str, Any], str]:
    """Resolve one shared Feishu record without widening personal access."""
    from app.gateway.deps import get_thread_store

    thread_store = get_thread_store(request)
    for owner_user_id in _shared_feishu_owner_ids():
        record = await thread_store.get(thread_id, user_id=owner_user_id)
        if record is not None and _is_shared_feishu_record(record):
            return record, owner_user_id
    raise HTTPException(status_code=404, detail=f"Feishu conversation {thread_id} not found")


async def _clear_thread_context_for_owner(
    request: Request,
    thread_id: str,
    *,
    user_id: str | None = None,
) -> ThreadClearContextResponse:
    """Clear one owned thread while preserving metadata and its channel mapping."""
    from app.gateway.deps import get_thread_store
    ...
    if record.get("status") == "busy":
        raise HTTPException(status_code=409, detail="A running conversation cannot be cleared")
    ...
    try:
        ...
        logger.exception("Failed to clear checkpoints for thread %s", sanitize_log_param(thread_id))
        raise HTTPException(status_code=500, detail="Failed to clear conversation context") from None

    try:
        removed_events = await get_run_event_store(request).delete_by_thread(thread_id)
    except Exception:
        logger.exception("Failed to clear message history for thread %s", sanitize_log_param(thread_id))
        raise HTTPException(status_code=500, detail="Failed to clear conversation history") from None

    await thread_store.update_metadata(
        thread_id,
        {"conversation_context_cleared_at": now_iso()},
        user_id=user_id,
    )
    return ThreadClearContextResponse(success=True, removed_events=removed_events)
...
async def delete_thread_data(thread_id: str, request: Request) -> ThreadDeleteResponse:
    ...
    record_result = thread_store.get(thread_id, user_id=user_id) if thread_store is not None else None
    record = await record_result if inspect.isawaitable(record_result) else None
    workflow = record.get("metadata", {}).get("bug_workflow") if isinstance(record, dict) else None
    if isinstance(workflow, dict):
        await _cancel_bug_workflow_before_delete(
            request,
            record,
            user_id=user_id,
        )
        ...
        for owned_thread_id in reversed(_bug_workflow_thread_ids(record)):
            # Early Bug Workbench reroutes could persist worker IDs longer than
            # today's canonical 64-character contract. They remain valid
            # metadata/checkpoint keys, but must never be resolved as host paths.
            try:
                validate_thread_id(owned_thread_id)
            except ValueError:
                logger.info(
                    "Skipped local data cleanup for legacy Bug worker %s",
                    sanitize_log_param(owned_thread_id),
                )
            else:
                _delete_thread_data(owned_thread_id, user_id=user_id)
            if checkpointer is not None and hasattr(checkpointer, "adelete_thread"):
                await checkpointer.adelete_thread(owned_thread_id)
            await event_store.delete_by_thread(owned_thread_id)
            await thread_store.delete(owned_thread_id, user_id=user_id)
        return ThreadDeleteResponse(
            success=True,
            message=f"Stopped and deleted Bug Workbench task {thread_id}",
        )
...
@router.post("/{thread_id}/clear-context", response_model=ThreadClearContextResponse)
@require_permission("threads", "write", owner_check=True, require_existing=True)
async def clear_thread_context(thread_id: ThreadId, request: Request) -> ThreadClearContextResponse:
    """Erase retained messages and checkpoints while keeping the thread listed.

    This is intentionally distinct from deleting a thread: channel operators can
    retain its Feishu source and title in the maintenance view, while removing
    all DeerFlow-held conversational context before the next incoming message.
    """
    return await _clear_thread_context_for_owner(
        request,
        thread_id,
        user_id=get_effective_user_id(),
    )
...
@router.get("/feishu-conversations", response_model=list[ThreadResponse])
@require_permission("threads", "read")
async def list_shared_feishu_conversations(request: Request) -> list[ThreadResponse]:
    """List shared Feishu conversations and visible Bug workflow roots.

    The endpoint exposes ordinary channel conversations plus only the root
    record of every Feishu-originated Bug Workbench run. It never exposes the
    internal router, specialist, or note-writer threads, and it never
    broadens visibility of a member's personal DeerFlow threads.
    """
    from app.gateway.deps import get_thread_store
    ...
    for owner_user_id in _shared_feishu_owner_ids():
        owner_rows = await thread_store.search(limit=1000, user_id=owner_user_id)
        for row in owner_rows:
            thread_id = row.get("thread_id")
            if not isinstance(thread_id, str) or thread_id in seen_thread_ids:
                continue
            seen_thread_ids.add(thread_id)
            rows.append(row)
    rows.sort(key=lambda row: str(row.get("updated_at", "")), reverse=True)
    # Retain every root run, including retries for the same Bug. Operators use
    # this view to inspect and delete duplicate test runs. Worker threads stay
    # hidden; the root record is the only visible unit of a Bug task.
    display_rows = [row for row in rows if _is_shared_feishu_record(row)]
    return [
        ThreadResponse(
            ...
            values={"title": row["display_name"]} if row.get("display_name") else {},
            interrupts={},
        )
...
@router.get("/feishu-conversations/{thread_id}/messages")
@require_permission("threads", "read")
async def list_shared_feishu_conversation_messages(thread_id: ThreadId, request: Request) -> list[dict]:
    """Return visible messages for one shared service-account Feishu thread."""
    record, owner_user_id = await _get_shared_feishu_thread(request, thread_id)
    rows = await get_run_event_store(request).list_messages(
        thread_id,
        limit=200,
    ...
    for row in rows:
        ...
        if not isinstance(content, dict) or content.get("type") not in {"human", "ai"}:
            continue
    ...
    # Conversations that predate ``run_events.backend: db`` have durable
    # checkpoints but no event timeline. Show their visible checkpoint history
    # as a read-only migration fallback; new turns use the event-store path.
    try:
        ...
        messages = snapshot.values.get("messages") if isinstance(snapshot.values, dict) else None
        serialized = serialize_channel_values_for_api({"messages": messages or []}).get("messages", [])
        fallback_created_at = coerce_iso(
            getattr(snapshot, "created_at", None) or record.get("updated_at", ""),
        )
        return [
            {"content": message, "created_at": fallback_created_at}
            for message in serialized
            if isinstance(message, dict) and message.get("type") in {"human", "ai"} and not (isinstance(message.get("additional_kwargs"), dict) and message["additional_kwargs"].get("hide_from_ui") is True)
        ]
    except Exception:
        logger.exception("Failed to read checkpoint fallback for Feishu conversation %s", sanitize_log_param(thread_id))
        return []


@router.post("/feishu-conversations/{thread_id}/clear-context", response_model=ThreadClearContextResponse)
@require_permission("threads", "write")
async def clear_shared_feishu_conversation_context(thread_id: ThreadId, request: Request) -> ThreadClearContextResponse:
    """Clear one visible conversation or an entire hidden-worker Bug task."""
    record, owner_user_id = await _get_shared_feishu_thread(request, thread_id)
    removed_events = 0
    for owned_thread_id in _shared_feishu_record_thread_ids(record):
        try:
            result = await _clear_thread_context_for_owner(
                request,
                owned_thread_id,
                user_id=owner_user_id,
            )
            ...
            # A workflow can stop before a worker has materialized a thread
            # record. That absent worker has no retained data to clear.
            if exc.status_code == 404 and owned_thread_id != thread_id:
                continue
            raise
    return ThreadClearContextResponse(success=True, removed_events=removed_events)


@router.delete("/feishu-conversations/{thread_id}", response_model=ThreadDeleteResponse)
@require_permission("threads", "delete")
async def delete_shared_feishu_conversation(thread_id: ThreadId, request: Request) -> ThreadDeleteResponse:
    """Delete one visible conversation or an entire Bug task's local records."""
    record, owner_user_id = await _get_shared_feishu_thread(request, thread_id)
    await _cancel_bug_workflow_before_delete(
        request,
        record,
    ...
    try:
        ...
        thread_store = get_thread_store(request)
        # Delete workers before their root; the root's metadata is the only
        # surviving public handle for the task until this loop finishes.
        for owned_thread_id in reversed(_shared_feishu_record_thread_ids(record)):
            _delete_thread_data(owned_thread_id, user_id=owner_user_id)
            if hasattr(checkpointer, "adelete_thread"):
                await checkpointer.adelete_thread(owned_thread_id)
            await get_run_event_store(request).delete_by_thread(owned_thread_id)
            await thread_store.delete(owned_thread_id, user_id=owner_user_id)
    except Exception:
        logger.exception("Failed to delete shared Feishu conversation %s", sanitize_log_param(thread_id))
        raise HTTPException(status_code=500, detail="Failed to delete shared Feishu conversation") from None
    return ThreadDeleteResponse(success=True, message=f"Deleted shared Feishu conversation {thread_id}")
