...
class FeishuChannel(Channel):
    ...
    @staticmethod
    def _mention_value(mention: Any, field: str) -> str | None:
        """Read a field from either the SDK model or a decoded event mapping."""
        value = mention.get(field) if isinstance(mention, dict) else getattr(mention, field, None)
        return value.strip() if isinstance(value, str) and value.strip() else None

    @staticmethod
    def _contains_broadcast_marker(text: str) -> bool:
        """Detect Feishu's observed ``@所有人`` transport marker."""
        return bool(re.search(r"(?:^|\s)@_all(?:\s|$)", text))

    @classmethod
    def _replace_plain_text_mention_keys(cls, text: str, mentions: Any) -> str:
        """Replace Feishu's ``@_user_N`` transport markers with display names.

        Text-message events encode every mention as an ordinal marker.  The
        accompanying ``message.mentions`` list maps that marker to the person
        or bot the user actually mentioned.  Sending the marker to the LLM is
        both confusing and leaks a transport implementation detail.
        """
        if not isinstance(mentions, (list, tuple)):
            return text

        for mention in mentions:
            key = cls._mention_value(mention, "key")
            name = cls._mention_value(mention, "name")
            if not key or not name:
                continue
            display_name = name.lstrip("@").strip()
            if display_name:
                text = text.replace(key, f"@{display_name}")
        return text

    @staticmethod
    def _strip_leading_group_mention_marker(text: str) -> str:
        """Remove the bot mention that caused a group event to be delivered.

        With Feishu's ``group_at_msg`` permission an inbound group message is
        delivered because this bot was mentioned.  The common ``@bot task``
        form therefore starts with the SDK marker ``@_user_N``.  That marker is
        routing noise, not part of the user's task.
        """
    ...
    async def _send_card_message(self, msg: OutboundMessage) -> None:
        ...
        if source_message_id:
            ...
            if msg.is_final:
                ...
                if not msg.metadata.get("main_assistant_incomplete"):
    ...
    def _schedule_mention_only_reply(self, message_id: str) -> None:
        """Reply deterministically when a group member only @mentions the bot."""
        if self._main_loop and self._main_loop.is_running():
            fut = asyncio.run_coroutine_threadsafe(self._reply_to_mention_only(message_id), self._main_loop)
            fut.add_done_callback(lambda f, mid=message_id: self._log_future_error(f, "mention_only_reply", mid))
        else:
            logger.warning("[Feishu] main loop not running, cannot reply to mention-only message")

    async def _reply_to_mention_only(self, message_id: str) -> None:
        await self._add_reaction(message_id, "OK")
        await self._reply_card(message_id, "成有翰已就绪。请发送 Bug 编号或网页查询关键词。")
    ...
    def _on_message(self, event) -> None:
        ...
        try:
            ...
            if chat_type == "group" and self._contains_broadcast_marker(text):
                logger.info(
                    "[Feishu] ignoring group broadcast mention: chat_id=%s, msg_id=%s, sender=%s",
                    chat_id,
                    msg_id,
                    sender_id,
                )
            ...
            # Feishu text events encode @mentions as ``@_user_N`` and provide
            # the human-readable mapping separately in ``message.mentions``.
            # A group event was delivered because this bot was mentioned, so
            # the first marker is the routing mention rather than user input.
            if "text" in content and chat_type == "group":
                text = self._strip_leading_group_mention_marker(text)
                if not text and not files_list:
                    self._schedule_mention_only_reply(msg_id)
                    return
            if "text" in content:
                text = self._replace_plain_text_mention_keys(text, mentions)
