...
from app.channels.manager import DEFAULT_BUG_WORKBENCH_URL, DEFAULT_GATEWAY_URL, DEFAULT_LANGGRAPH_URL, ChannelManager
...
class ChannelService:
    ...
    def __init__(
        ...
        *,
        ...
        bug_workbench_owner_user_id = feishu_config.get("bug_workbench_owner_user_id") if isinstance(feishu_config, dict) else None
        bug_workbench_url = feishu_config.get("bug_workbench_url") if isinstance(feishu_config, dict) else None
        ...
        self.manager = ChannelManager(
            ...
            bug_workbench_owner_user_id=bug_workbench_owner_user_id,
            bug_workbench_url=bug_workbench_url if isinstance(bug_workbench_url, str) else DEFAULT_BUG_WORKBENCH_URL,
    ...
    async def stop(self) -> None:
        """Finalize accepted messages before disconnecting outbound channels."""
...
async def deliver_scheduled_run_result(task: dict[str, Any], record: Any) -> None:
    """Deliver an opted-in scheduled Agent result through an existing channel.

    Delivery routing is stored with the scheduled task rather than inferred
    from a fresh execution thread. Ordinary scheduled tasks remain unchanged.
    """
    schedule_spec = task.get("schedule_spec")
    delivery = schedule_spec.get("delivery") if isinstance(schedule_spec, dict) else None
    if not isinstance(delivery, dict):
        return
    channel_name = delivery.get("channel_name")
    chat_id = delivery.get("chat_id")
    if not isinstance(channel_name, str) or not channel_name.strip() or not isinstance(chat_id, str) or not chat_id.strip():
        return
    status = getattr(getattr(record, "status", None), "value", getattr(record, "status", None))
    if status == "success":
        text = record.last_ai_message
        if not isinstance(text, str) or not text.strip():
            logger.warning("Scheduled task %s produced no final text; outbound delivery skipped", task.get("id"))
            return
        text = text.strip()
    else:
        detail = str(getattr(record, "error", None) or "未知原因").strip()
        text = f"定时任务「{task.get('title') or task.get('id') or '未命名任务'}」执行失败：{detail}"
    service = get_channel_service()
    if service is None:
        raise RuntimeError("channel service is not available")
    await service.bus.publish_outbound(
        OutboundMessage(
            ...
            text=text,
            is_final=True,
            owner_user_id=str(delivery.get("owner_user_id") or task.get("user_id") or "") or None,
            metadata={"scheduled_task_id": task.get("id")},
        )
    )
