...
class ScheduledTaskService:
    def __init__(
        ...
        *,
        ...
        deliver_result: Callable[[dict[str, Any], RunRecord], Awaitable[None]] | None = None,
    ) -> None:
    ...
    async def handle_run_completion(self, record: RunRecord) -> None:
        ...
        if terminal_status in {"success", "failed"} and self._deliver_result is not None:
            try:
                await self._deliver_result(task, record)
            except Exception:
                # Delivery is a channel concern. A transient outbound failure
                # must not rewrite the Agent outcome or make a recurring task
                # terminal. Interrupted runs stay silent because they are
                # normally an intentional operator action.
                logger.exception("Scheduled task %s result delivery failed", task_id)
