...
def ask_clarification_tool(
    ...
    """Ask for user-owned information needed by an available execution path.

    A missing tool, skill, API, or permission is a capability gap, not missing
    user information. Do not use this tool to compensate for such a gap by
    requesting IDs, exports, credentials, or manual work. For a reusable
    capability gap, follow the skill-system workflow when creation is available:
    explain the missing capability and use one concise suggestion question to
    ask whether the user wants a general, parameterized skill created. After
    agreement, the skill creation workflow may use this tool for genuinely
    ...
    - You have confirmed that an available tool or skill can execute the task after the answer
    ...
    - Do not ask for task parameters when the actual blocker is an unavailable capability
