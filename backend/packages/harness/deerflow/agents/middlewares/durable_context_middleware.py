...
_AUTHORITY_CONTRACT = "\n".join(
    [
        ...
        "The active run's direct user messages and tool results take precedence over every historical summary or reference below.",
...
def _render_durable_context_data(summary_text: str | None, ledger: list, skills: list) -> str:
    ...
    if summary_text:
        ...
        data_parts.append(
            "## Historical conversation summary (background only; current run wins)\n"
            f"{escape(bounded_summary, quote=False)}"
        )
...
class DurableContextMiddleware(AgentMiddleware[AgentState]):
    ...
    def _capture(self, state: AgentState, runtime: Runtime | None) -> dict | None:
        ...
        skills = extract_skills(
            ...
            read_tool_names=self._skill_read_tool_names,
            run_id=run_id,
        )
        if skills:
