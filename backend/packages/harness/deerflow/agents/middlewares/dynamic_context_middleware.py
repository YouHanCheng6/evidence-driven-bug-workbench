...
from deerflow.runtime.context_keys import CURRENT_RUN_PRE_EXISTING_MESSAGE_IDS_KEY, SKIP_MEMORY_CONTEXT_KEY
...
class DynamicContextMiddleware(AgentMiddleware):
    ...
    def _build_full_reminder(self, runtime: Runtime | None = None) -> tuple[str, str | None]:
        ...
        skip_memory = bool(runtime_context.get(SKIP_MEMORY_CONTEXT_KEY)) if isinstance(runtime_context, dict) else False
        memory_context = (
            _get_memory_context(
                self._agent_name,
            ...
            if injection_enabled and not skip_memory
            else ""
        )
