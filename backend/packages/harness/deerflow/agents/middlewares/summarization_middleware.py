...
from langchain_core.messages import AnyMessage, HumanMessage, RemoveMessage, ToolMessage, get_buffer_string, trim_messages
...
from deerflow.agents.middlewares.skill_context import read_skill_entry_metadata
...
def _pre_existing_message_ids(runtime: Runtime | None) -> frozenset[str] | None:
    """Return the server-owned checkpoint boundary, or ``None`` when unavailable.

    An explicitly empty set is meaningful: the thread had no messages before
    this run, so every message currently in state belongs to the active run.
    """
    context = getattr(runtime, "context", None)
    raw = context.get(CURRENT_RUN_PRE_EXISTING_MESSAGE_IDS_KEY) if isinstance(context, dict) else None
    if not isinstance(raw, (set, frozenset, list, tuple)):
        return None
    return frozenset(str(message_id) for message_id in raw if message_id)


def _neutralize_skill_bodies(messages: list[AnyMessage]) -> list[AnyMessage]:
    """Keep a skill read as history without turning its body into lasting policy."""
    neutralized: list[AnyMessage] = []
    for message in messages:
        metadata = read_skill_entry_metadata(getattr(message, "additional_kwargs", None)) if isinstance(message, ToolMessage) else None
        if metadata is None:
            neutralized.append(message)
            continue
        description = metadata.get("description") or ""
        content = f"Skill file read: {metadata['path']}. Historical reference only; re-read and activate it in a future run before applying its instructions."
        if description:
            content += f" Declared scope: {description}"
        neutralized.append(message.model_copy(update={"content": content}))
    return neutralized
...
class DeerFlowSummarizationMiddleware(SummarizationMiddleware):
    ...
    def _build_summary_prompt(self, messages_to_summarize: list[AnyMessage], previous_summary: str | None = None) -> str | None:
        ...
        # message objects.
        formatted_messages = get_buffer_string(_neutralize_skill_bodies(trimmed_messages))
    ...
    def _prepare_compaction(
        ...
        *,
        ...
        # Automatic compaction may summarize checkpoint history, but it must
        # never fold the active run's user request, confirmations, skill reads,
        # or tool results into historical prose.  Those direct messages are the
        # authoritative task state until this run ends.  Manual /compact keeps
        # its explicit whole-history semantics via ``force=True``.
        pre_existing_ids = _pre_existing_message_ids(runtime)
        if not force and pre_existing_ids is not None:
            current_run_boundary = next(
                (
                    index
                    for index, message in enumerate(messages)
                    if not getattr(message, "id", None) or str(message.id) not in pre_existing_ids
                ),
                len(messages),
            )
            cutoff_index = min(cutoff_index, current_run_boundary)
        if cutoff_index <= 0:
            return None
    ...
    def compact_state(
        ...
        *,
        ...
        """
        prepared = self._prepare_compaction(state, runtime, force=force)
        if prepared is None:
            return None
