...
{escaped_skill_content}
    ...
    def _resolve_secret_bindings(self, request: ModelRequest, activation: _Activation | None, *, hook: str) -> None:
        ...
        - the most recent slash activation of this run (persisted as a source on
          ...
          the run anyway;
        - skills the model loaded during this run (recorded in
    ...
    def _in_context_secret_sources(self, request: ModelRequest, registry: dict[str, Skill]) -> list[tuple[str, tuple[SecretRequirement, ...]]]:
        ...
        Entries are references to skills the model actually loaded. When the
        runtime supplies a run id, historical entries from earlier runs are
        ignored. Each current-run entry is re-validated against the live registry so a skill that
        ...
        current_run_id = str(context.get("run_id")) if isinstance(context, dict) and context.get("run_id") else None
        for entry in entries:
            if not isinstance(entry, dict):
                continue
            if current_run_id is not None and entry.get("run_id") != current_run_id:
                continue
