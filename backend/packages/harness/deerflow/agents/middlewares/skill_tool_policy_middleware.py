    ...
    def _active_policy(self, request: ModelRequest | ToolCallRequest) -> _PolicySignature:
        ...
        for entry in entries:
            if not isinstance(entry, dict):
                continue
            # skill_context is durable discovery history. Only a skill read during
            # this run may constrain this run's tools. Keep the legacy behavior
            # when integrations do not supply a run id.
