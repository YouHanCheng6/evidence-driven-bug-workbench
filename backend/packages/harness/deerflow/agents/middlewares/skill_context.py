...
def render_skill_context(entries: list[SkillEntry]) -> str:
    """Render historical skill references as data, not active instructions."""
    if not entries:
        return ""

    lines = ["## Previously loaded skill references (historical only; re-read in this run before applying)"]
