...
from deerflow.agents.middlewares.tool_call_limit_middleware import ToolCallLimitMiddleware
...
_MAIN_ASSISTANT_AGENT_NAME = "personal-assistant"


def _custom_agent_extra_tools(
    agent_name: str | None,
    *,
    skill_management_enabled: bool,
    is_webhook_channel: bool,
    update_tool,
    bug_workbench_tool,
) -> list:
    """Return agent adapters that must not enter the global tool catalog."""
    tools = []
    if agent_name == _MAIN_ASSISTANT_AGENT_NAME:
    ...
    if agent_name and not is_webhook_channel:
...
def build_middlewares(
    ...
    *,
    ...
    # Inert for ordinary conversations; trusted bounded workflows opt in through
    # runtime context. Keep the native activation/policy/context chain contiguous.
    middlewares.append(ToolCallLimitMiddleware())

    cfg = _get_runtime_config(config)
    # A bounded, single-turn repair already carries a durable backend repair
    # packet. Compacting that short turn can discard the read-before-write mark
    # immediately before the approved edit, forcing the model into a read loop.
    # Only trusted internal callers can set this flag.
    if not cfg.get("skip_summarization", False):
    ...
    if token_budget_config.enabled and agent_name != _MAIN_ASSISTANT_AGENT_NAME:
...
def _make_lead_agent(config: RunnableConfig, *, app_config: AppConfig):
    ...
    from deerflow.tools.builtins import read_bug_workbench_tool, setup_agent, update_agent
    ...
    is_webhook_channel = channel_name in _WEBHOOK_CHANNELS or (agent_name == _MAIN_ASSISTANT_AGENT_NAME and channel_name == "feishu")
    extra_tools = _custom_agent_extra_tools(
        agent_name,
        skill_management_enabled=bool(agent_config and agent_config.skill_management_enabled),
        is_webhook_channel=is_webhook_channel,
        update_tool=update_agent,
        bug_workbench_tool=read_bug_workbench_tool,
    )
    # Default lead agent (unchanged behavior)
    raw_tools = get_available_tools(model_name=model_name, groups=agent_config.tool_groups if agent_config else None, subagent_enabled=subagent_enabled, app_config=resolved_app_config)
    configured_tools = raw_tools + extra_tools
    if agent_name == _MAIN_ASSISTANT_AGENT_NAME:
        configured_tools = list({tool.name: tool for tool in configured_tools}.values())
    if non_interactive:
        configured_tools = [tool for tool in configured_tools if tool.name not in _NON_INTERACTIVE_DISABLED_TOOL_NAMES]
    authorization_candidates = [*configured_tools]
    if skill_setup.describe_skill_tool:
    ...
    return create_agent(
        ...
        system_prompt=apply_prompt_template(
            ...
            user_id=resolved_user_id,
            skill_names=skill_setup.skill_names or None,
            skill_management_enabled=any(tool.name == "skill_manage" for tool in authorized_tools),
        ),
        state_schema=get_thread_state_schema(mode),
    )
