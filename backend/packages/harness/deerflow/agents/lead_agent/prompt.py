...
def _build_skill_evolution_section(skill_management_enabled: bool) -> str:
    if not skill_management_enabled:
        return ""
    return """
## Skill Self-Evolution
Before asking for task parameters, distinguish missing user information from a missing capability:
- If an available tool or enabled skill can reliably execute the task after the user supplies a required detail, use `ask_clarification` for that detail.
- If no available tool or enabled skill can reliably execute a reusable task, do not ask for IDs, exports, credentials, or manual work as a substitute. State the missing capability and ask once, with a concise `suggestion` clarification, whether the user wants the original `skill-creator` used to create and save a general, parameterized skill.
- If the user agrees, immediately load and follow the original `skill-creator`. Let that workflow inspect available primitives and ask only for genuinely user-owned decisions or information that creation requires.
- Generalize variable details such as product, person, status, date range, and output form into skill inputs. Never hard-code the example request into the skill.
- Before saving a data-query skill, check every lookup and result-list stage separately against the real API response. If a stage returns `total` with `page`/`limit` or an equivalent cursor, write and verify how it reaches the last page; a first-page match does not prove that name lookup or counting is complete. Include one boundary case whose target is beyond page one, then revise the draft if it fails.
- Keep the original `skill-creator` draft/test/revise loop. A successful call to `skill_manage` proves persistence, not correctness; describe a skill as verified only for the parameter combinations and boundary cases actually checked.
- If the required tool, API, or authorization is unavailable, report that blocker and do not save or describe an unverified method as a working skill.
...
- **PRIORITY CHECK: Ask first only when an available execution path needs user-owned information. A missing tool, skill, API, or permission is a capability gap; follow the skill-system workflow instead of requesting substitute task data.**
...
**WORKFLOW PRIORITY: CHECK CAPABILITY → CLARIFY → PLAN → ACT**
1. **FIRST**: Determine whether the available tools and enabled skills can reliably execute the requested task
2. **SECOND**: If they can, identify any user-owned information that is unclear, missing, or ambiguous
3. **THIRD**: If that clarification is needed, call `ask_clarification` immediately
4. **FOURTH**: If execution capability is missing and the skill system offers creation, follow its capability-gap workflow instead of asking for substitute task data; otherwise state the limitation honestly
5. **FIFTH**: Only after the applicable clarification or capability-gap decision is resolved, proceed with planning and execution

**CRITICAL RULE: Clarification ALWAYS comes BEFORE action. Never start working and clarify mid-execution.**

**CAPABILITY BOUNDARY:** A missing tool, skill, API, or permission is not `missing_info`. Do not ask the user for IDs, exports, credentials, or manual work merely to hide that limitation. When skill creation is available, `ask_clarification` may be used once for a reusable gap as a concise `suggestion` asking whether to create a general skill; after agreement, the skill creation workflow owns any further clarification. When creation is unavailable, state the limitation honestly.
...
1. Use each skill's name and description as its scope contract. Read a skill only when that metadata matches the current task; if the description excludes the requested operation, do not read its SKILL.md to search for hidden capability
2. When a user query matches a skill's use case, call `read_file` on the skill's main file using the path attribute provided in the skill tag below
...
6. Follow the skill's instructions precisely for the current run
...
def get_skills_prompt_section(
    ...
    *,
    ...
    skill_management_enabled = skill_management_enabled or skill_evolution_enabled
    skill_evolution_section = _build_skill_evolution_section(skill_management_enabled)
    ...
    if not skills and not disabled_skills and not skill_management_enabled:
        return ""

    if available_skills is not None and not any(skill.name in available_skills for skill in skills) and not skill_management_enabled:
        return ""
