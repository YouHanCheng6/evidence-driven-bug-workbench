...
from deerflow.runtime.context_keys import SKIP_MEMORY_CONTEXT_KEY
...
def memory_flush_hook(event: SummarizationEvent) -> None:
    ...
    if (
        not get_memory_config().enabled
        or not event.thread_id
        or (isinstance(runtime_context, dict) and runtime_context.get(SKIP_MEMORY_CONTEXT_KEY))
    ):
        return
