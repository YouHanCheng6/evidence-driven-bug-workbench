    ...
    *,
    ...
    try:
        ...
        if record.abort_event.is_set():
            ...
            main_cap_error = (
                "主助手已达到运行边界，本次处理未完成。"
                if runtime_context is not None and runtime_context.get("agent_name") == "personal-assistant" and stop_reason in {"token_capped", "loop_capped", "safety_capped", "model_length_capped", "subagent_limit_capped"}
                else None
            )
            ...
            cancel_action = await run_manager.set_status_if_not_cancelled(
                run_id,
                RunStatus.error if delivery_error or main_cap_error else RunStatus.success,
                error=delivery_error or main_cap_error,
