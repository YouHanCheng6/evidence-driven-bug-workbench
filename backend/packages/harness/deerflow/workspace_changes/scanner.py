...
# A stdio MCP subprocess receives a thread-local temporary directory here. Its
# package installs/unpacks are runtime plumbing, not agent-authored workspace
# edits, so they must never appear in the workspace-change UI or delivery
# checks. Keep this path-specific: other .mcp configuration remains visible.
EXCLUDED_RELATIVE_DIR_PATHS = {Path(".mcp/tmp")}
    ...
    *,
    ...
    for root in roots:
        ...
        for dirpath, dirnames, filenames in os.walk(root.host_path, followlinks=False):
            ...
            dirnames[:] = [dirname for dirname in dirnames if dirname not in excluded_dir_names and not _is_excluded_relative_dir(root.host_path, current_dir / dirname) and not (current_dir / dirname).is_symlink()]
...
def _is_excluded_relative_dir(root_path: Path, directory: Path) -> bool:
    """Return whether a nested runtime-only directory must be pruned."""
    try:
        return directory.relative_to(root_path) in EXCLUDED_RELATIVE_DIR_PATHS
    except ValueError:
        return False
