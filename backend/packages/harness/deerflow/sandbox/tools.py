...
from deerflow.runtime.context_keys import (
    BUG_UI_ALLOWED_SOURCE_PATHS_CONTEXT_KEY,
    ...
    BUG_UI_MAP_SOURCE_CANDIDATES_CONTEXT_KEY,
    BUG_UI_PENDING_STATE_SEARCH_TERMS_CONTEXT_KEY,
    BUG_UI_PROBLEM_FOCUS_CONTEXT_KEY,
    BUG_UI_TRACED_ANCHOR_IDENTIFIERS_CONTEXT_KEY,
)
...
from deerflow.sandbox.ui_source_navigation import explicit_relative_source_paths, source_anchor_candidates_for_search
...
def _ui_bug_runtime_context(runtime: Runtime) -> dict | None:
...
def _normalized_ui_source_path(path: str) -> str:
    return path.replace("\\", "/").rstrip("/")


def _ui_bug_entry_source_path(runtime: Runtime) -> str | None:
    context = _ui_bug_runtime_context(runtime)
    value = context.get(BUG_UI_ENTRY_SOURCE_PATH_CONTEXT_KEY) if context is not None else None
    return value if isinstance(value, str) and value else None


def _ui_bug_source_path_error(runtime: Runtime, path: str) -> str | None:
    entry = _ui_bug_entry_source_path(runtime)
    if entry is None:
        return None
    context = _ui_bug_runtime_context(runtime)
    allowed = context.get(BUG_UI_ALLOWED_SOURCE_PATHS_CONTEXT_KEY) if context is not None else None
    allowed_paths = {_normalized_ui_source_path(entry)}
    if isinstance(allowed, list):
        allowed_paths.update(_normalized_ui_source_path(item) for item in allowed if isinstance(item, str) and item)
    if _normalized_ui_source_path(path) in allowed_paths:
        return None
    return f"Error: UI source navigation is scoped to the confirmed implementation entry {entry} and source paths explicitly referenced by already-read code."


def _confirm_ui_bug_entry_from_matches(runtime: Runtime, matches: list[GrepMatch]) -> str | None:
    context = _ui_bug_runtime_context(runtime)
    if context is None or context.get(BUG_UI_ENTRY_SOURCE_PATH_CONTEXT_KEY):
        return None
    source_paths = {_normalized_ui_source_path(match.path) for match in matches}
    candidates = context.get(BUG_UI_MAP_SOURCE_CANDIDATES_CONTEXT_KEY)
    mapped_paths = {_normalized_ui_source_path(path) for path in candidates or [] if isinstance(path, str)}
    if len(source_paths) != 1 or not source_paths.issubset(mapped_paths):
        return None
    source_path = next(iter(source_paths))
    context[BUG_UI_ENTRY_SOURCE_PATH_CONTEXT_KEY] = source_path
    context[BUG_UI_ALLOWED_SOURCE_PATHS_CONTEXT_KEY] = [source_path]
    focus = context.get(BUG_UI_PROBLEM_FOCUS_CONTEXT_KEY)
    focus_text = focus.strip() if isinstance(focus, str) and focus.strip() else "使用本轮已有 Bug 和附件证据"
    return f"已确认实现入口：{source_path}\n当前问题重点：{focus_text}"


def _remember_ui_bug_explicit_source_candidates(runtime: Runtime, matches: list[GrepMatch]) -> None:
    """Project literal caller imports as shallow candidates, never as owners."""
    context = _ui_bug_runtime_context(runtime)
    if context is None or context.get(BUG_UI_ENTRY_SOURCE_PATH_CONTEXT_KEY):
        return
    projected: list[str] = []
    for match in matches:
        for candidate in explicit_relative_source_paths(match.path, match.line):
            if candidate not in projected:
                projected.append(candidate)
    if not projected:
        return
    existing = [item for item in context.get(BUG_UI_MAP_SOURCE_CANDIDATES_CONTEXT_KEY, []) if isinstance(item, str)]
    for candidate in projected:
        if candidate not in existing:
            existing.append(candidate)
    context[BUG_UI_MAP_SOURCE_CANDIDATES_CONTEXT_KEY] = existing


def _remember_ui_bug_source_relationships(runtime: Runtime, path: str, content: str) -> None:
    context = _ui_bug_runtime_context(runtime)
    if context is None or _ui_bug_entry_source_path(runtime) is None or _ui_bug_source_path_error(runtime, path) is not None:
        return
    existing = context.get(BUG_UI_ALLOWED_SOURCE_PATHS_CONTEXT_KEY)
    paths = [item for item in existing or [] if isinstance(item, str)]
    for related in explicit_relative_source_paths(path, content):
        if related not in paths:
            paths.append(related)
    context[BUG_UI_ALLOWED_SOURCE_PATHS_CONTEXT_KEY] = paths


def _remember_ui_bug_search_anchors(runtime: Runtime, path: str, pattern: str, matches: list[GrepMatch]) -> None:
    context = _ui_bug_runtime_context(runtime)
    entry = _ui_bug_entry_source_path(runtime)
    if context is None or entry is None:
        return
    if _normalized_ui_source_path(path) != _normalized_ui_source_path(entry):
        return
    try:
        content = read_current_file_content(runtime, path)
    except (FileNotFoundError, SandboxError):
        return
    candidates = source_anchor_candidates_for_search(content, pattern, tuple(match.line for match in matches))
    visited = {item for item in context.get(BUG_UI_TRACED_ANCHOR_IDENTIFIERS_CONTEXT_KEY, []) if isinstance(item, str)}
    candidates = tuple(candidate for candidate in candidates if candidate not in visited)
    if not candidates:
        return
    pending_state_terms = tuple(item.lower() for item in context.get(BUG_UI_PENDING_STATE_SEARCH_TERMS_CONTEXT_KEY, []) if isinstance(item, str))
    if pending_state_terms:
        state_render_candidates = tuple(
            candidate
            for candidate in candidates
            if re.search(r"render|control", candidate, re.IGNORECASE) and any(term in candidate.lower() for term in pending_state_terms)
        )
        if state_render_candidates:
            candidates = state_render_candidates
    context.pop(BUG_UI_PENDING_STATE_SEARCH_TERMS_CONTEXT_KEY, None)
    context[BUG_UI_ANCHOR_SOURCE_PATH_CONTEXT_KEY] = path
    existing = context.get(BUG_UI_ANCHOR_CANDIDATES_CONTEXT_KEY)
    merged = [item for item in existing or [] if isinstance(item, str)]
    for candidate in candidates:
        if candidate not in merged:
            merged.append(candidate)
    context[BUG_UI_ANCHOR_CANDIDATES_CONTEXT_KEY] = merged
...
def bash_tool(runtime: Runtime, command: str, description: str = "") -> str:
    ...
    Args:
        ...
        description: Optional short display label for this command.
    """
    try:
...
def ls_tool(runtime: Runtime, path: str, description: str = "") -> str:
    ...
    Args:
        ...
        description: Optional short display label for this directory listing.
    """
    try:
...
def glob_tool(
    ...
    Args:
        ...
        description: Optional short display label for this path search.
...
def grep_tool(
    ...
    Args:
        ...
        description: Optional short display label for this content search.
    ...
    try:
        ...
        if _is_disabled_skill_path(path, user_id=user_id):
            skill_name = _extract_skill_name_from_skills_path(path) or "unknown"
            return f"Error: Skill '{skill_name}' is disabled. Access to its files is blocked. Enable the skill in settings before using it."
        ui_bug_context = _ui_bug_runtime_context(runtime)
        if ui_bug_context is not None:
            max_results = min(max_results, _BUG_UI_GREP_MAX_RESULTS)
            locked_path_error = _ui_bug_source_path_error(runtime, path)
            if locked_path_error:
                return locked_path_error
        sandbox = ensure_sandbox_initialized(runtime)
        ensure_thread_directories_exist(runtime)
        requested_path = path
        effective_max_results = _resolve_max_results(
        ...
        result = _format_grep_results(requested_path, matches, truncated)
        if ui_bug_context is None or not matches:
            return result
        entry_prompt = _confirm_ui_bug_entry_from_matches(runtime, matches)
        _remember_ui_bug_explicit_source_candidates(runtime, matches)
        _remember_ui_bug_search_anchors(runtime, requested_path, pattern, matches)
        return f"{result}\n\n{entry_prompt}" if entry_prompt else result
    except SandboxError as e:
        return f"Error: {e}"
    except FileNotFoundError:
        return f"Error: Directory not found: {requested_path}"
...
def read_file_tool(
    ...
    Args:
        ...
        description: Optional short display label for this file read.
    ...
    try:
        ...
        _remember_ui_bug_source_relationships(runtime, requested_path, result)
        return result
    except SandboxError as e:
        return f"Error: {e}"
    except FileNotFoundError:
...
def write_file_tool(
    ...
    Args:
        ...
        description: Optional short display label for this file write.
...
def str_replace_tool(
    ...
    Args:
        ...
        description: Optional short display label for this replacement.
