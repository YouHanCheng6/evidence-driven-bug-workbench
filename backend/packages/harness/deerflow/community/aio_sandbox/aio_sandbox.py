...
class AioSandbox(Sandbox):
    ...
    def execute_command(
        ...
        Args:
            ...
            env: Optional per-call environment variables (request-scoped secrets,
                ...
                unchanged.
            timeout: Optional hard deadline using one-shot bash.exec plus a
                request deadline. None retains the persistent-shell default.

        Returns:
            The output of the command.
        """
        if timeout is not None:
            return self._execute_bounded_command(command, env, timeout)
    ...
    def _execute_bounded_command(self, command: str, env: dict[str, str] | None, timeout: float) -> str:
        """Opt-in one-shot execution; the ordinary persistent-shell path stays unchanged."""
        _validate_extra_env(env)
        if not self._lock.acquire(timeout=5):
            return "Error: 沙箱正在处理其他命令，未执行本次操作。请勿重复等待。"
        try:
            result = self._client.bash.exec(
                command=command,
                env=env or {},
                hard_timeout=timeout,
                request_options={"timeout_in_seconds": timeout + 5},
            )
            data = result.data if result else None
            if data is None:
                return "Error: 沙箱未返回命令结果，操作未确认完成。"
            output = data.stdout or ""
            if data.stderr:
                output += f"\nStd Error:\n{data.stderr}"
            exit_code = getattr(data, "exit_code", None)
            if exit_code not in (None, 0):
                output += f"\nError: 命令退出码 {exit_code}，操作未确认成功。"
            return output or "(no output)"
        except Exception as e:
            return f"Error: 沙箱命令未完成（{type(e).__name__}），不要将其视为成功。"
        finally:
            self._lock.release()
