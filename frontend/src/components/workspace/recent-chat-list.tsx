...
export function RecentChatList() {
  ...
  const browserLocation = search ? `${pathname}?${search}` : pathname;
  ...
  const recentChatListModel = useMemo(
    () =>
      buildThreadListModel([
        threadListModel.threads.filter(isRecentChatThread),
      ]),
    [threadListModel.threads],
  );
  const { threads } = recentChatListModel;
  const displayedThreads = useMemo(() => {
    if (
      !threadIdFromPath ||
      recentChatListModel.displayedThreads.some(
        (thread) => thread.thread_id === threadIdFromPath,
      )
    ) {
      return recentChatListModel.displayedThreads;
    }
    const activeThread = threadListModel.byId.get(threadIdFromPath);
    return activeThread && isRecentChatThread(activeThread)
      ? [...recentChatListModel.displayedThreads, activeThread]
      : recentChatListModel.displayedThreads;
  }, [recentChatListModel, threadIdFromPath, threadListModel.byId]);

  const sentinelRef = useRef<HTMLDivElement | null>(null);
  useEffect(() => {
    const element = sentinelRef.current;
    if (!element || !hasNextPage || !recentChatListModel.canLoadMore) {
      return;
    }
    ...
    hasNextPage,
    isFetchingNextPage,
    recentChatListModel.canLoadMore,
  ]);
  ...
  const [bugThreadPendingDelete, setBugThreadPendingDelete] =
          ...
          ? () => {
              if (bugWorkflowRoot) {
                window.localStorage.removeItem(
                  "deerflow:bug-workbench:last-workflow",
                );
                window.location.assign("/workspace/bugs");
                return;
              }
  ...
  const handleDelete = useCallback(
    (thread: AgentThread) => {
      if (isBugWorkbenchRootThread(thread)) {
        setBugThreadPendingDelete(thread);
        return;
      }
      deleteSelectedThread(thread);
    },
  ...
  return (
    <>
          ...
          <SidebarMenu>
            ...
            >
              <VirtualThreadList
                ...
                renderItem={(thread) => {
                  const threadPath = pathOfThread(thread);
                  const isActive = threadPath === browserLocation;
                    ...
                    >
                      <SidebarMenuButton isActive={isActive} asChild>
                        ...
                        >
                          ...
                          {bugWorkflowRoot && (
                            <span className="bg-muted text-muted-foreground ml-auto inline-flex h-5 shrink-0 items-center rounded-md px-1.5 text-[10px] font-medium">
                              Bug
                            </span>
                          )}
                          {channelSource && (
              ...
              />
              {hasNextPage && recentChatListModel.canLoadMore && (
                <>
                  <Button
                    variant="ghost"
                    size="sm"
      ...
      <Dialog
        open={bugThreadPendingDelete !== null}
        onOpenChange={(open) => {
          if (!open) setBugThreadPendingDelete(null);
        }}
      >
        <DialogContent className="sm:max-w-[460px]">
          <DialogHeader>
            <DialogTitle>删除这次 Bug 工作台任务？</DialogTitle>
          </DialogHeader>
          <p className="text-muted-foreground text-sm leading-6">
            删除会先停止仍在运行的分析或修复，再清理本次任务及内部专家会话。已经写入禅道的备注和已经产生的代码修改不会自动撤销。
          </p>
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setBugThreadPendingDelete(null)}
            >
              {t.common.cancel}
            </Button>
            <Button
              variant="destructive"
              onClick={() => {
                if (bugThreadPendingDelete) {
                  deleteSelectedThread(bugThreadPendingDelete);
                }
                setBugThreadPendingDelete(null);
              }}
            >
              停止并删除
            </Button>
