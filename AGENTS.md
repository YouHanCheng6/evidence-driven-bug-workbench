...
## Personal showcase boundary

This directory is the sanitized personal customization edition, not only the
upstream framework. Read `README.md` and `docs/personal/SOURCE_MAP.md` first.
Preserve the upstream attribution and distinguish customized whole-file copies
from newly authored line counts. Sample repository names, routing conventions,
products and assignees are fictional and do not describe a company source tree.
Never add real source mirrors, tickets, logs, attachments, traces or credentials.
Runtime compatibility is not established. Validate offline changes with
`python3.12 scripts/check_public_release.py` and
`python3.12 -m unittest discover -s tests/public_release -v`; these do not substitute
for end-to-end workflow tests. See `docs/personal/REDACTION_AND_LIMITATIONS.md`.
