...
_reclaim_repo_port_before_start() {
    local port=$1
    local elapsed=0
    local timeout=5
    ...
    # A just-stopped dev server can leave a descendant (most commonly
    # next-server) listening for a few seconds after its parent exits. Give a
    # process which is already shutting down a chance to release the socket,
    # then reclaim only DeerFlow-owned survivors. An unrelated process is
    # deliberately left alone and remains a clear startup error below.
    while _is_port_listening "$port" && [ "$elapsed" -lt 2 ]; do
        sleep 1
        elapsed=$((elapsed + 1))
    done
    ...
    elapsed=0
    while _is_port_listening "$port" && [ "$elapsed" -lt "$timeout" ]; do
        sleep 1
        elapsed=$((elapsed + 1))
    done
}

_report_port_listener_details() {
    local port=$1
    local pid
    ...
    while IFS= read -r pid; do
        [ -n "$pid" ] || continue
        ps -p "$pid" -o pid=,ppid=,user=,command= 2>/dev/null | sed 's/^/    /' || true
    done < <(lsof -nP -iTCP:"$port" -sTCP:LISTEN -t 2>/dev/null || true)
}
...
run_service() {
    ...
    if _is_port_listening "$port"; then
        _reclaim_repo_port_before_start "$port"
    fi
    ...
    if _is_port_listening "$port"; then
        ...
        _report_port_listener_details "$port"
        cleanup 1
    fi
